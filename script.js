...
// v1.1: auto presence, chat profile tab, nicknames, group admin
// ============================================================
...
let friends = [];
let friendsData = {};
let nicknames = {};   // { otherUid: "nickname" }
let blockedUsers = {}; // { otherUid: true }
let pendingRequests = [];
let onlineUsers = {};
let unreadCounts = {};
let userGroups = {};
...
let _contextMenuType = null; // 'dm' | 'group'
...
function getInitial(name) {
    if (!name) return '?';
    // Get first "grapheme" so emoji renders correctly
...
// Display name with nickname override + emoji safe
...
// AUTO STATUS — replaces manual status
...
function listenPresence() {
    ...
    // Refresh "Active Xm ago" text every 60s
    setInterval(() => {
        renderAllFriends();
        if (currentDmUser) updateDmHeader();
        if ($('#chatProfileStatus')) updateChatProfileStatus();
    }, 60000);
}
...
function listenNicknames() {
    nicknamesRef.child(userId).on('value', snap => {
        ...
        renderAllFriends();
        if (currentDmUser) updateDmHeader();
        if (currentGroup) updateGroupHeader();
    });
}
...
// ============================================================
// CONTEXT MENU (fixed pin/mute labels)
// ============================================================
...
function updateContextMenuLabels() {
    ...
    if (!_contextMenuTarget) return;
    const key = _contextMenuType === 'dm' ? 'dm_' + _contextMenuTarget : 'group_' + _contextMenuTarget;
    const pinLabel = $('#ctxPinLabel');
    const muteLabel = $('#ctxMuteLabel');
    if (pinLabel) pinLabel.textContent = isChatPinned(key) ? 'Unpin' : 'Pin';
    if (muteLabel) muteLabel.textContent = isChatMuted(key) ? 'Unmute' : 'Mute';
...
// ============================================================
// DELETE HELPERS — really wipe data
// ============================================================
async function deleteDmConversation(uid) {
    ...
    try {
        const key = dmKey(userId, uid);
        // Remove messages, reactions, unread
...
async function deleteGroupConversation(gid) {
    if (!currentGroup || currentGroup.id !== gid) {
        // Not currently open; just hide from view until membership change
        showToast('Cannot delete', 'Open the group first to leave it.');
        return;
    }
    // Group "delete" = leave the group
    const ok = await showModal({
        title: 'Leave group?',
...
function openChatProfile() {
    ...
    cpThemeValue.textContent = 'Default';
...
dmHeaderClickable.addEventListener('click', () => {
...
// Nickname editor
$('#cpRowNickname').addEventListener('click', async () => {
    if (!currentDmUser) return;
    const uid = currentDmUser.uid;
    const current = nicknames[uid] || '';
    const val = await showModal({
        title: 'Set nickname',
        text: 'This nickname is only visible to you.',
        icon: 'fas fa-tag',
        input: true,
    ...
    if (trimmed) {
        await nicknamesRef.child(userId).child(uid).set(trimmed);
    } else {
...
// Chat theme (simple preset)
$('#cpRowTheme').addEventListener('click', async () => {
    if (!currentDmUser) return;
    const val = await showModal({
        title: 'Chat theme',
        text: 'Choose a background tint for this conversation.',
        icon: 'fas fa-circle-half-stroke',
        input: true,
        inputPlaceholder: 'default / sage / rose / sky / midnight',
        inputValue: localStorage.getItem('chat_theme_' + currentDmUser.uid) || 'default',
        confirmText: 'Apply'
    });
    if (val === false) return;
    const themes = ['default', 'sage', 'rose', 'sky', 'midnight'];
    const chosen = String(val || '').toLowerCase().trim();
    if (!themes.includes(chosen)) {
        showToast('Invalid theme', 'Available: ' + themes.join(', '));
        return;
    }
    localStorage.setItem('chat_theme_' + currentDmUser.uid, chosen);
    applyChatTheme();
    cpThemeValue.textContent = chosen.charAt(0).toUpperCase() + chosen.slice(1);
    showToast('Theme applied', `Chat theme set to ${chosen}.`);
});

function applyChatTheme() {
    const msgs = document.getElementById('dmMessages');
    if (!msgs) return;
    msgs.classList.remove('chat-theme-sage', 'chat-theme-rose', 'chat-theme-sky', 'chat-theme-midnight');
    if (!currentDmUser) return;
    const t = localStorage.getItem('chat_theme_' + currentDmUser.uid) || 'default';
    if (t !== 'default') msgs.classList.add('chat-theme-' + t);
}
...
// ============================================================
// MEMBERS MODAL — with kick buttons for owner
// ============================================================
...
// Group members button in old header is gone — no-op safety
...
$$('.settings-nav-item').forEach(item => {
    item.addEventListener('click', () => {
        ...
        $$('.settings-nav-item').forEach(i => i.classList.toggle('active', i === item));
        $$('.settings-section').forEach(s => s.classList.toggle('active', s.dataset.section === section));
    });
});
